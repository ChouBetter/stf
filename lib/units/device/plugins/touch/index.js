...
module.exports = syrup.serial()
  ...
  .define(function (options, adb, router, minitouch, flags) {
    ...
    TouchConsumer.prototype.touchDown = function (point) {
      console.log(point)
      console.log(this.banner.maxX)
      console.log(this.banner.maxY)
      this._queueWrite(function () {
        return this._write(util.format(
          'd %s %s %s %s\n', point.contact, Math.floor(this.touchConfig.origin.x(point) * this.banner.maxX), Math.floor(this.touchConfig.origin.y(point) * this.banner.maxY), Math.floor((point.pressure || 0.5) * this.banner.maxPressure)
        ))
      })
    }
    ...
    TouchConsumer.prototype._failLimitExceeded = function (limit, time) {
      ...
      this.emit('error', new Error(util.format(
        'Failed more than %d times in %dms', limit, time
      )))
    }
    ...
    TouchConsumer.prototype._stopService = function (output) {
      ...
      function kill(signal) {
        ...
        return Promise.all([
            output.waitForEnd(), adb.shell(options.serial, ['kill', signum, pid])
            ...
            switch (args[0]) {
              ...
              default:
                throw new Error(util.format(
                  'Unexpected output "%s", expecting version line', chunk
                ))
            }
          })
      }
        ...
        return parser.readLine()
          .then(function (chunk) {
            ...
            switch (args[0]) {
              ...
              default:
                throw new Error(util.format(
                  'Unknown output "%s", expecting limits line', chunk
                ))
            }
          })
      }
        ...
        return parser.readLine()
          .then(function (chunk) {
            ...
            switch (args[0]) {
              ...
              default:
                throw new Error(util.format(
                  'Unexpected output "%s", expecting pid line', chunk
                ))
            }
          })
      }

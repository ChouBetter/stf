...
function getDevices(req, res) {
  ...
  if ("adminMode" in req.user && req.user.adminMode == 1)
    $rootScope.adminMode = true;

  var allowALL = false;
  var allowDevices = [];
  if (
    "allowDevices" in req.user &&
    Array.isArray(req.user.allowDevices) &&
    req.user.allowDevices.length > 0
  ) {
    allowDevices = req.user.allowDevices;
    allowALL = allowDevices[0] == "ALL";
  }
    ...
    .then(function (cursor) {
      return Promise.promisify(cursor.toArray, cursor)().then(function (list) {
        ...
        list.forEach(function (device) {
          ...
          if (allowALL || allowDevices.includes(responseDevice.serial))

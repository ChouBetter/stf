...
module.exports = function (options) {
  ...
  app.post("/app/api/v1/fbscript/insert", express.json(), function (req, res) {
    console.log(req.user.email);
    //console.log(req.body);
    req.body["owner"] = req.user.email;
    console.log(req.body);

    request.post('http://localhost:8090/taskmanager/insert', {
      json: req.body
    }, (error, response, body) => {
  ...
  app.get("/app/api/v1/fbscript", function (req, res) {
    console.log(req.user.name);
    var reqURl = 'http://localhost:8090//taskmanager/scripts';
  ...
  app.get("/app/api/v1/fbscript/status", function (req, res) {
    console.log(req.user.email);
    request.post('http://localhost:8090/taskmanager/status', {
      json: {
  ...
  app.get("/app/api/v1/pyLDAP/add/:account/:password/:allow", function (
    req,
    res
  ) {
    console.log(req.user.name);
      ...
      function (err, conn, callback) {
        ...
        r.db("stf")
          ...
          .run(conn, (err, result) => {
            ...
            console.log(result);
            res.status(200).json({
              allowDevices: result && result.hasOwnProperty("allowDevices") ?
                result.allowDevices : [],
            });
          });
      }
  ...
  app.get("/app/api/v1/iscsi", function (req, res) {
    var reqURl = `http://${req.query.host}:19000/${req.query.path}`;
    console.log("URL:", reqURl);
    request(reqURl, function (error, response, body) {
      console.error("error:", error); // Print the error if one occurred
      console.log("statusCode:", response && response.statusCode); // Print the response status code if a response was received
